...
def read_truth_if_available(path: Optional[str], template: pd.DataFrame) -> Optional[pd.DataFrame]:
    candidates: List[Path] = []
    if path:
        candidates.append(Path(path))
    for default_name in ["test.csv", "solution.csv", "ground_truth.csv", "truth.csv"]:
        candidates.append(Path(default_name))
    for p in candidates:
        if p.exists():
            df = pd.read_csv(p, low_memory=False)
            if set(ID_COLS + ["frame_id", "x", "y"]).issubset(df.columns):
                return df[ID_COLS + ["frame_id", "x", "y"]].copy()
    # If the template itself has x,y, treat those as truth before dropping for prediction.
    if set(ID_COLS + ["frame_id", "x", "y"]).issubset(template.columns):
        return template[ID_COLS + ["frame_id", "x", "y"]].copy()
    return None
...
def local_predict_and_evaluate(args: argparse.Namespace) -> None:
    ...
    truth = read_truth_if_available(args.truth, template)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--test", type=str, default=None, help="Optional local Kaggle test template CSV.")
    parser.add_argument("--truth", type=str, default=None, help="Optional truth CSV with x,y for error calculation.")
